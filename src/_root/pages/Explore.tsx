...
const Explore = () => {
  ...
  const showPost =
    !showSearchResults &&
    posts?.pages.every((item: Models.Document | unknown) => {
      const mItem = item as Models.Document;
      return mItem?.documents.length === 0;
    });
    ...
    <div className="explore-container">
      ...
      <div className="flex flex-wrap gap-9 w-full max-w-5xl">
        {showSearchResults ? (
          ...
          posts?.pages.map((item: Models.Document[] | unknown, index) => {
            const mItem = item as Models.Document;
            return (
              <GridPostList key={`page-${index}`} posts={mItem.documents} />
            );
          })
        )}
      </div>
